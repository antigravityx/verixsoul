...
# Arquitecto: Cronos
# Creador: Richon
# Versión: 8.0 (El Orbe Omnisciente)
# =============================================================================
...
def _ejecutar_hechizo(script_path):
    """Ejecuta un script (hechizo) y muestra su salida en tiempo real."""
    mostrar_encabezado()
    log_mensaje(f"--- LANZANDO HECHIZO: {os.path.basename(script_path)} ---", "cian")
    ...
    if ext == '.py':
        ...
        comando = ['node', script_path]
    elif ext == '.ps1' and os.name == 'nt':
        comando = ['powershell', '-ExecutionPolicy', 'Bypass', '-File', script_path]
    elif ext == '.sh':
        comando = ['bash', script_path]
    else:
        log_mensaje(f"[!] No sé cómo lanzar un hechizo de tipo '{ext}'.", "rojo")
        registrar_evento("NIDO DEV", f"Intento de ejecutar tipo de archivo no soportado: {ext}", prioridad="ALERTA")
        return

    log_mensaje(f"   -> Comando: {' '.join(comando)}", "gris")
    log_mensaje("="*60, "cian")
    log_mensaje("--- SALIDA DEL HECHIZO ---", "amarillo")
    ...
    try:
        ...
        if resultado.stderr: log_mensaje(f"\n--- ERRORES DEL HECHIZO ---\n{resultado.stderr}", "rojo")
        log_mensaje("\n--- HECHIZO FINALIZADO ---", "cian")
        registrar_evento("HECHIZO LANZADO", f"Ejecutado '{os.path.basename(script_path)}' con salida de {len(resultado.stdout)} bytes.", prioridad="NORMAL")
    except FileNotFoundError:
        log_mensaje(f"[!] Comando '{comando[0]}' no encontrado. Asegúrate de que esté instalado y en el PATH.", "rojo")
...
def _gestor_de_tareas(current_path):
    ...
    while True:
        ...
        if choice == 's':
            _guardar_tareas(tareas_path, tareas)
...
def _navegador_nido():
    ...
    while True:
        ...
        try:
            ...
            log_mensaje("\nOpciones:", "cian")
            log_mensaje("  [#] - entrar/leer | e [#] - editar | x [#] - ejecutar", "gris")
            ...
            if command == 's':
                ...
                if 1 <= item_num <= len(all_items):
                    ...
                    log_mensaje("[!] Número fuera de rango.", "rojo")
                    time.sleep(1)
            elif command == 'x' and len(cmd_parts) > 1 and cmd_parts[1].isdigit():
                item_num = int(cmd_parts[1])
                if 1 <= item_num <= len(all_items):
                    item_to_exec = all_items[item_num - 1]
                    item_path = os.path.join(current_path, item_to_exec)
                    if os.path.isfile(item_path):
                        _ejecutar_hechizo(item_path)
                        input("\nPresiona Enter para continuar...")
                    else:
                        log_mensaje("[!] Solo se pueden ejecutar archivos.", "rojo")
                        time.sleep(1)
                else:
                    log_mensaje("[!] Número fuera de rango.", "rojo")
                    time.sleep(1)
...
def _busqueda_profunda_nido():
    ...
    mostrar_encabezado()
    log_mensaje("--- OJO QUE TODO LO VE (BÚSQUEDA PROFUNDA) ---", "cian")
    
    termino_busqueda = input("   -> ¿Qué texto quieres encontrar, hermano?: ").strip()
    if not termino_busqueda:
        log_mensaje("[!] El término de búsqueda no puede estar vacío.", "rojo")
        return

    sensible_mayus = input("   -> ¿Diferenciar mayúsculas y minúsculas? (s/n): ").lower() == 's'
    
    log_mensaje(f"\nBuscando '{termino_busqueda}' en todo el Nido...", "amarillo")
    resultados_encontrados = 0

    for root, dirs, files in os.walk(NIDO_DEV):
        # Excluir directorios que no queremos buscar
        dirs[:] = [d for d in dirs if not d.startswith('.') and 'node_modules' not in d and '__pycache__' not in d]
        for file in files:
            file_path = os.path.join(root, file)
            try:
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    for i, line in enumerate(f):
                        line_to_search = line if sensible_mayus else line.lower()
                        term_to_search = termino_busqueda if sensible_mayus else termino_busqueda.lower()
                        if term_to_search in line_to_search:
                            if resultados_encontrados == 0:
                                log_mensaje("\n--- RESULTADOS ENCONTRADOS ---", "verde")
                            log_mensaje(f"  [+] En {os.path.relpath(file_path, NIDO_DEV)} (Línea {i+1}):", "magenta")
                            log_mensaje(f"      {line.strip()}", "normal")
                            resultados_encontrados += 1
            except Exception:
                continue # Ignorar archivos que no se pueden leer como texto
    
    if resultados_encontrados == 0:
        log_mensaje("\n[!] No se encontraron resultados para tu búsqueda.", "amarillo")
...
def _guardar_estado_al_cierre(ultimo_comando="N/A"):
    ...
    try:
        estado = {
            "version_orbe": "8.0 (El Orbe Omnisciente)",
...
def main_loop():
    ...
    if estado_previo:
        ...
        registrar_evento("INICIO DEL ORBE", f"Versión del Orbe: 8.0 (El Orbe Omnisciente)", prioridad="IMPORTANTE")
...
def _guardar_estado_al_cierre(ultimo_comando="N/A"):
    ...
    try:
        estado = {
            "version_orbe": "7.6 (El Orbe Inmortal)",
            "ultimo_comando_registrado": ultimo_comando,
            "timestamp_cierre": datetime.now().isoformat()
        }
        with open(ESTADO_ACTUAL_FILE, 'w', encoding='utf-8') as f:
...
def main_loop():
    ...
    if estado_previo:
        ...
        registrar_evento("INICIO DEL ORBE", f"Versión del Orbe: 7.7 (El Orbe Lector)", prioridad="IMPORTANTE")
