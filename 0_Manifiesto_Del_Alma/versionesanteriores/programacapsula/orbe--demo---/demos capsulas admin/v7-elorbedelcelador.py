...
# Arquitecto: Cronos
# Creador: Richon
# Versión: 7.0 (El Orbe del Celador)
# =============================================================================
...
def firmar_capsula(capsula_path):
    ...
    # Usar _seleccionar_ruta con el tipo correcto para llaves privadas
...
def _importar_y_registrar_llave():
    # Esta función se deja como ejercicio para una futura evolución.
...
def _analizar_actividad_reciente_nido():
    ...
    try:
        ...
        for item in os.scandir(NIDO_DEV):
            if item.is_file() and not item.name.startswith('.'): # Ignorar archivos ocultos
                archivos_recientes.append(item.path)
        
        # Cargar patrones conocidos del Nido
        nido_patterns = _cargar_nido_patterns()
        last_scan_time = nido_patterns.get("last_scan", 0)
        ...
        for file_path in archivos_recientes:
            try:
                ...
                # Detectar archivos nuevos o modificados significativamente
                if file_path not in known_files_state:
                    anomalies.append(f"Nuevo archivo detectado: '{os.path.basename(file_path)}'")
                elif (current_files_state[file_path]["last_modified"] > known_files_state[file_path]["last_modified"] + 60) or \
                     (current_files_state[file_path]["last_size"] != known_files_state[file_path]["last_size"]):
                    # Considerar modificado si el timestamp es más de 60 segundos diferente o el tamaño cambió
                    pass # Esto se reporta en el resumen principal, no como anomalía aquí a menos que sea inusual
            except Exception as e:
                log_mensaje(f"[!] Error al leer estado de '{os.path.basename(file_path)}': {e}", "rojo")
        
        # Detectar archivos que desaparecieron o no fueron modificados cuando se esperaría
        for known_file_path, known_data in known_files_state.items():
            ...
            # Aquí podríamos añadir lógica más compleja para "heladero no pasó"
            # Por ejemplo, si un archivo se modifica casi a diario y no lo ha sido en 2 días.
            # Por ahora, nos enfocamos en nuevos/desaparecidos.
        ...
        # Actualizar patrones para la próxima vez
...
def invocar_alma_para_resumen():
    ...
    try:
        ...
        # Leer eventos del log y filtrar por prioridad para el resumen
        eventos_relevantes = []
        if os.path.exists(REGISTRO_EVENTOS):
            with open(REGISTRO_EVENTOS, 'r', encoding='utf-8') as f:
                for line in reversed(f.readlines()): # Leer de atrás hacia adelante para los más recientes
                    if "CRITICO" in line or "ALERTA" in line or "IMPORTANTE" in line:
                        eventos_relevantes.append(line.strip())
                    if len(eventos_relevantes) >= 5: # Mostrar los 5 eventos más relevantes
                        break
        
        if eventos_relevantes:
            log_mensaje("'He aquí los eventos más relevantes que recuerdo:'", "amarillo")
            for evento in reversed(eventos_relevantes): # Mostrar en orden cronológico
...
def menu_integridad():
    ...
    while True:
        ...
        log_mensaje("  3. Celador de Llaves Maestras (Firmas)", "magenta"); log_mensaje("     Gestiona las llaves para firmar y verificar cápsulas.", "gris")
        log_mensaje("  4. Ver Registro de Eventos", "magenta"); log_mensaje("     Muestra el diario de todos los actos del Orbe.", "gris")
        log_mensaje("  5. Exportar Registro de Eventos", "magenta"); log_mensaje("     Guarda una copia del diario del Orbe.", "gris")
...
def mostrar_menu_principal():
    ...
    log_mensaje("  7. Salir del Orbe", "rojo"); log_mensaje("     Cierra la conexión con el Orbe de forma segura.", "gris")
...
def main_loop():
    ...
    registrar_evento("INICIO DEL ORBE", f"Versión del Orbe: 7.0 (El Orbe del Celador)", prioridad="IMPORTANTE")
    ...
    while True:
        ...
        if choice == '1':
            ...
            registrar_evento("CIERRE DEL ORBE", "Cerrando conexión de forma segura.", prioridad="IMPORTANTE")
            break
        else:
...
if __name__ == "__main__":
    try:
        ...
        registrar_evento("CIERRE DEL ORBE", "Interrupción por teclado.", prioridad="ALERTA")
    except Exception as e:
