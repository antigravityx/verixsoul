...
def _navegador_nido():
    ...
    while True:
        ...
        log_mensaje(f"Ubicación actual: {os.path.relpath(current_path, SANTUARIO_RAIZ)}", "amarillo")
        ...
        try:
            ...
            log_mensaje("\nCarpetas:", "cian")
            if not dirs: log_mensaje("  (ninguna)", "gris")
            for i, dirname in enumerate(dirs):
                log_mensaje(f"  {i+1}. [ {dirname} ]", "magenta")

            log_mensaje("\nArchivos:", "cian")
            if not files: log_mensaje("  (ninguno)", "gris")
            # Encabezado de la tabla de archivos
            if files:
                log_mensaje(f"  {'#':<3} {'Nombre':<40} {'Tamaño':>10} {'Modificado':>16}", "gris")
            ...
            for i, filename in enumerate(files):
                ...
                log_mensaje(f"  {len(dirs) + i + 1:<3}. {filename:<40} {size_str:>10} {date_str:>16}", "azul")

            log_mensaje("\nOpciones:", "cian")
            log_mensaje("  [número] - para entrar a carpeta o leer archivo", "gris")
            ...
            if choice == 's':
                ...
                _renombrar_nido_item(current_path, dirs + files)
                input("\nPresiona Enter para continuar...")
            elif choice.isdigit():
                choice_num = int(choice)
                if 1 <= choice_num <= len(dirs): # Si es una carpeta
                    current_path = os.path.join(current_path, dirs[choice_num - 1])
                elif len(dirs) < choice_num <= len(dirs) + len(files): # Si es un archivo
                    file_index = choice_num - len(dirs) - 1
                    _leer_archivo_nido(os.path.join(current_path, files[file_index]))
                    input("\nPresiona Enter para continuar...")
                else:
...
def gestor_nido_dev():
    ...
    while True:
        ...
        log_mensaje("  n. Navegar y administrar el Nido", "magenta"); log_mensaje("     Explora, crea archivos y gestiona tus proyectos.", "gris")
