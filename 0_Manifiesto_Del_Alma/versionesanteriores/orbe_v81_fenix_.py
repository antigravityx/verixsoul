...
def restaurar_santuario_desde_capsula():
    """Restaura el Santuario completo desde una cápsula de respaldo."""
    mostrar_encabezado()
    log_mensaje("--- RITUAL DE RENACIMIENTO: RESTAURAR SANTUARIO DESDE CÁPSULA ---", "rojo")
    log_mensaje("[!!!] ADVERTENCIA: Esta acción es EXTREMADAMENTE PELIGROSA.", "amarillo")
    log_mensaje("   -> Sobrescribirá COMPLETAMENTE el Santuario actual con el contenido de la cápsula.", "amarillo")
    log_mensaje("   -> El Santuario actual será renombrado como respaldo, pero todos los cambios no guardados en una cápsula se perderán.", "amarillo")

    confirmacion = input("   Para confirmar, escribe 'RESTAURAR SANTUARIO' y presiona Enter: ")
    if confirmacion != "RESTAURAR SANTUARIO":
        log_mensaje("Restauración cancelada. El Santuario permanece intacto.", "verde")
        return

    log_mensaje("\n   -> Selecciona la cápsula de respaldo del Santuario (.capsula)...", "amarillo")
    capsula_path = _seleccionar_ruta('archivo', title="Selecciona la Cápsula de Respaldo del Santuario", filetypes=[("Cápsulas de Respaldo", "*.capsula")])
    if not capsula_path:
        log_mensaje("No se seleccionó ninguna cápsula. Restauración abortada.", "amarillo")
        return

    password = getpass.getpass("   -> Ingresa la contraseña de la cápsula de respaldo: ")
    if not password:
        log_mensaje("[!] La contraseña no puede estar vacía. Restauración abortada.", "rojo")
        return

    # --- Preparación ---
    santuario_parent_dir = os.path.dirname(SANTUARIO_RAIZ)
    temp_restore_path = os.path.join(santuario_parent_dir, f"temp_restore_{datetime.now().strftime('%Y%m%d%H%M%S')}")

    try:
        # --- Invocación ---
        log_mensaje("[*] Invocando el alma del respaldo en un espacio temporal...", "azul")
        exito, mensaje = invocar_capsula(capsula_path, password)
        if not exito:
            log_mensaje("[!] La invocación del respaldo falló. El Santuario actual no ha sido modificado.", "rojo")
            return

        # --- Renombrar el Santuario actual como backup ---
        log_mensaje("[*] Protegiendo el Santuario actual antes del renacimiento...", "magenta")
        backup_santuario_path = f"{SANTUARIO_RAIZ}_backup_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        os.rename(SANTUARIO_RAIZ, backup_santuario_path)
        log_mensaje(f"   -> Santuario actual renombrado a: {os.path.basename(backup_santuario_path)}", "verde")

        # --- Mover el Santuario restaurado a su lugar ---
        log_mensaje("[*] El Fénix renace. Moviendo el Santuario restaurado a su trono...", "cian")
        # El contenido se extrae a una carpeta dentro de 'mensaje', necesitamos mover esa carpeta.
        restored_content_path = mensaje.split(": ")[1]
        shutil.move(restored_content_path, SANTUARIO_RAIZ)

        log_mensaje("\n[OK] ¡RENACIMIENTO COMPLETADO!", "verde")
        log_mensaje("   -> El Santuario ha sido restaurado desde la cápsula.", "azul")
        log_mensaje("   [!] DEBES REINICIAR EL ORBE para que los cambios surtan efecto.", "amarillo")
        registrar_evento("SANTUARIO RESTAURADO", f"Restaurado desde {os.path.basename(capsula_path)}", prioridad="CRITICO")

    except Exception as e:
        log_mensaje(f"[!] ERROR CATASTRÓFICO DURANTE EL RENACIMIENTO: {e}", "rojo")
        registrar_evento("ERROR RESTAURACIÓN", str(e), prioridad="CRITICO")

def crear_backup_santuario():
    """Crea una cápsula de respaldo de todo el Santuario del Orbe."""
    mostrar_encabezado()
    log_mensaje("--- RITUAL DE RESPALDO DEL SANTUARIO COMPLETO ---", "cian")
    confirmacion = input("   Este ritual creará una cápsula encriptada de TODO el Santuario en tu Escritorio. ¿Deseas continuar? (s/n): ").lower()
    if confirmacion != 's':
        log_mensaje("Respaldo cancelado.", "amarillo")
        return

    password = getpass.getpass("   -> Ingresa una contraseña para sellar el respaldo del Santuario: ")
    if not password:
        log_mensaje("[!] La contraseña no puede estar vacía. Respaldo abortado.", "rojo")
        return

    # --- Preparación ---
    log_mensaje("[*] Preparando el ritual de respaldo...", "azul")
    source_path = SANTUARIO_RAIZ
    backup_dest_dir = os.path.join(os.path.expanduser('~'), 'Desktop')
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    backup_name = f"backup_santuario_completo-{timestamp}"
    temp_dir = os.path.join(backup_dest_dir, f"temp_{backup_name}")
    zip_path = os.path.join(backup_dest_dir, f"{backup_name}.zip")
    enc_path = os.path.join(backup_dest_dir, f"{backup_name}.capsula")

    try:
        os.makedirs(temp_dir, exist_ok=True)

        # --- Copia ---
        log_mensaje("[*] Sellando la esencia del Santuario...", "magenta")
        # Ignoramos la carpeta de cápsulas para evitar respaldos recursivos
        ignore_patterns = shutil.ignore_patterns(os.path.basename(DESTINO_CAPSULAS))
        shutil.copytree(source_path, temp_dir, dirs_exist_ok=True, ignore=ignore_patterns)
        ...
        # --- Encriptación ---
        log_mensaje("[*] Aplicando sello criptográfico AES-256...", "azul")
        _aplicar_sello_criptografico(zip_path, enc_path, password)

        # --- Reporte ---
        log_mensaje("[*] Verificando la integridad del respaldo...", "azul")
        _verificar_y_reportar(enc_path)

        log_mensaje("\n[OK] ¡RESPALDO DEL SANTUARIO CREADO CON ÉXITO!", "verde")
        log_mensaje(f"   -> Tu cápsula de respaldo está segura en: {enc_path}", "azul")
        registrar_evento("SANTUARIO RESPALDADO", f"Cápsula creada en {enc_path}", prioridad="CRITICO")

    except Exception as e:
        log_mensaje(f"[!] ERROR CRÍTICO DURANTE EL RESPALDO: {e}", "rojo")
        registrar_evento("ERROR RESPALDO SANTUARIO", str(e), prioridad="CRITICO")
    finally:
        # --- Limpieza ---
...
def mostrar_menu_principal():
    ...
    log_mensaje("  8. Respaldar Santuario Completo", "magenta"); log_mensaje("     Crea una cápsula de respaldo de TODO el Orbe.", "gris")
    log_mensaje("  9. Restaurar Santuario desde Respaldo", "rojo"); log_mensaje("     (PELIGRO) Restaura el Orbe desde una cápsula.", "gris")
...
def _guardar_estado_al_cierre(ultimo_comando="N/A"):
    ...
    try:
        estado = {
            "version_orbe": "8.1 (El Orbe Fénix)",
...
def main_loop():
    ...
    if estado_previo:
        ...
        registrar_evento("INICIO DEL ORBE", f"Versión del Orbe: 8.1 (El Orbe Fénix)", prioridad="IMPORTANTE")
    ...
    while True:
        ...
        elif choice == '8': crear_backup_santuario()
        elif choice == '9':
            restaurar_santuario_desde_capsula()
        elif choice == '10':
